...
The OutSplice function has 6 required inputs and 4 Optional Inputs:

The OutSplice_TCGA function has 5 required inputs and 4 Optional Inputs and should only be used if the data is in TCGA format:

```{r}
#OutSplice<-function(junction, RSEM, rawcounts, sample_labels, out_file_prefix, dir, filterSex=T, genome='Homo.sapiens', annotation='org.Hs.eg.db', TxDb='TxDb.Hspaiens.UCSC/hg38.knownGene')
#OutSplice_TCGA<-function(junction, RSEM, rawcounts, output_file_prefix, dir, filterSex=T, genome = 'Homo.sapiens', annotation = 'org.Hs.eg.db', TxDb = 'TxDb.Hsapiens.UCSC.hg38.knownGene')
```

  -junction: refers to the junction raw read counts file
  -RSEM: refers to normalized gene expression file. TCGA provides a quartile normalized RSEM expression file.
  -rawcounts: refers to the reads per million counts (RPM) for each sample
  -sample_labels: matrix of phenotypes (in Section 2.2)
  -out_file_prefix: user defined string for what the prefix of the output file should be named
  -dir: string containing the directory path to OutSplice
...
The Bioconductor GenomicRanges packages are used to assign each junction to a known gene. The user has the option in the main function to input which genome to use as the reference.
...
Offsets, which the user can specify, sets a minimum value relative to the normal samples in order to call a junction an outlier. In this example example, an outlier must have expression greater than 0.1 RPM in order to be called an outlier. Any outliers with expressions below  0.1 RPM are too low to be relevant for the analysis in this example.

```{r}
#allsamps=dotheogsa(Sample.data=junc.RPM.norm, PHENO=PHENO, offsets=0.00001, Fisher=T, correction='fdr', dir = dir)
```
